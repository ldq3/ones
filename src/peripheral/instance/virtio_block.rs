use crate::{ memory::AsRaw, peripheral::virtio::{ Hal, Result, Error, header::VirtIOHeader, queue::VirtQueue } };
use bitflags::*;
...
impl<H: Hal> VirtIOBlk<'_, H> {
    ...
    pub fn new(header: &'static mut VirtIOHeader) -> Result<Self> {
        ...
        Ok(VirtIOBlk {
            header,
            queue,
            _capacity: config.capacity.read() as usize,
        })
    }
    ...
    pub fn read_block(&mut self, block_id: usize, buf: &mut [u8]) -> Result {
        ...
        self.queue.add(&[req.as_raw()], &[buf, resp.as_raw_mut()])?;
    ...
    pub unsafe fn read_block_nb(
        ...
        let token = self.queue.add(&[req.as_raw()], &[buf, resp.as_raw_mut()])?;
        self.header.notify(0);
        Ok(token)
    }
...
impl AsRaw for BlkReq {}
impl AsRaw for BlkResp {}
